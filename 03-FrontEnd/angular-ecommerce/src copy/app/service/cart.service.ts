...
export class CartService {
  ...
  addToCart(theCartItem: CartItem) {
    ...
    if (this.cartItems.length > 0) {
      ...
      existingCartItem = this.cartItems.find(teempCartItem => teempCartItem.id === theCartItem.id);
  ...
  logCartData(totalPriceValue: number, totalQuantityValue: number) {
    console.log(`Contents of the part`);
  ...
  remove(theCartItem: CartItem) {
    ...
    //if found remeove the item from the array at the given index
    if (itemIndex > -1) {
